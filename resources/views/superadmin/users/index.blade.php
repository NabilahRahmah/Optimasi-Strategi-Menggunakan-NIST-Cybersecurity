            ...
            <table class="w-full text-sm border-collapse">
                <thead>
                    <tr class="border-b bg-muted/50">
                        <th class="h-12 px-6 text-left font-medium text-muted-foreground uppercase text-[10px]">Nama Karyawan</th>
                        <th class="h-12 px-6 text-left font-medium text-muted-foreground uppercase text-[10px]">Email</th>
                        <th class="h-12 px-6 text-left font-medium text-muted-foreground uppercase text-[10px]">Role / Jabatan</th>
                        ...
                        <tr class="border-b hover:bg-muted/50">
                            ...
                            <td class="p-6">
                                @if($karyawan->role == 'admin_super')
                                    <span class="px-2 py-1 bg-purple-100 text-purple-700 rounded-md text-xs font-bold uppercase">Super Admin</span>
                                @elseif($karyawan->role == 'admin')
                                    <span class="px-2 py-1 bg-blue-100 text-blue-700 rounded-md text-xs font-bold uppercase">Admin</span>
                                @elseif($karyawan->role == 'approver')
                                    <span class="px-2 py-1 bg-amber-100 text-amber-700 rounded-md text-xs font-bold uppercase">Approver</span>
                                @else
                                    <span class="px-2 py-1 bg-gray-100 text-gray-700 rounded-md text-xs font-bold uppercase">User (SOC)</span>
                                @endif
                            </td>
                            <td class="p-6 text-right">
                                <form action="{{ route('superadmin.users.destroy', $karyawan->user_id) }}"
                                      method="POST"
