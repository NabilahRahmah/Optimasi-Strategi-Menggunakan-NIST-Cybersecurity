...
<div class="space-y-6">
    <nav class="flex items-center gap-2 text-sm text-muted-foreground">
        ...
        <span>/</span>
        <span class="font-medium text-foreground">Edit Pertanyaan</span>
    </nav>

    <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
            <h1 class="text-2xl font-bold tracking-tight text-foreground">Edit Pertanyaan</h1>
            <p class="text-sm text-muted-foreground mt-1">
                Perbarui pertanyaan <span class="font-mono text-primary font-semibold">{{ $pertanyaan->kode_pertanyaan }}</span>
            </p>
        </div>
    ...
    <form action="{{ route('admin.pertanyaan.update', $pertanyaan->pertanyaan_id) }}" method="POST">
        @csrf @method('PUT')
                ...
                <div class="rounded-xl border bg-card shadow-sm p-6 space-y-5">
                        ...
                        <select name="kategori_id" required
                            ...
                            @foreach($kategoris as $kat)
                                <option value="{{ $kat->kategori_id }}"
                                    {{ old('kategori_id', $pertanyaan->kategori_id) == $kat->kategori_id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <input type="text" name="kode_pertanyaan"
                               value="{{ old('kode_pertanyaan', $pertanyaan->kode_pertanyaan) }}"
                    ...
                    <div>
                        ...
                        <textarea name="deskripsi" rows="4"
                                  class="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-primary/40 resize-none">{{ old('deskripsi', $pertanyaan->deskripsi) }}</textarea>
                    </div>
                </div>
